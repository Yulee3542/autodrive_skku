...
| [4. 점검](#4단계-환경-및-하드웨어-점검) | `check_env.py` / `run_tests.py` / `doctor.py` / `hw_test.py` | 주행 전 |
...
8. Foxglove에서 `/debug/lane_poi` 오버레이 확인 — 이제 원본 프레임이 아니라 BEV(원근변환)로 워프된 캔버스에 그린다(2026-07-23). 밴드 안에 차선 클러스터가 잡히고 청록색 lookahead 원이 차선을 따라가는지, 하단 텍스트의 `delta=`(조향각)가 커브에서 커지는지 확인. 안 잡히면 `ros2 param set /mission_node lane_poi.v_min_floor <값>`으로 조명에 맞게 라이브 튜닝(Otsu 적응 임계의 바닥값), 사다리꼴 자체(`lane_poi.src_frac`)가 안 맞으면 [Pure-Pursuit/BEV 캘리브레이션](#pure-pursuit-bev-캘리브레이션) 참고 ([실차 튜닝](#실차-튜닝-ros2-param))
...
| `/debug/lane_poi` | `sensor_msgs/CompressedImage` | BEV(원근변환) 캔버스 위 차선 인식 + Pure-Pursuit 오버레이 — 밴드/클러스터/코리도어 락/lookahead 지점/조향각 (road·t_parking 출차 후) |
...
└── tools/                              # check_env.py(환경 점검), run_tests.py(모듈별 on/off 테스트 러너,
                                         ...
                                         # doctor.py(실행 중인 시스템 단계별 진단 — 노드/토픽/
                                         #   아두이노/카메라/라이다 생존, 비파괴),
                                         # pick_camera.py(전방/후방 카메라 확인 후 안정 식별자로 저장),
...
ros2 param get /mission_node lane_poi.v_min_floor
ros2 param set /mission_node lane_poi.v_min_floor 90         # Otsu 적응 임계 바닥값 — /debug/lane_poi로 확인
ros2 param set /mission_node lane_poi.px_per_m 200.0         # BEV 픽셀→미터 캘리브 (조향각 크기에 영향)
ros2 param set /mission_node lane_poi.deg_per_pulse 2.5      # 조향 펄스 1회당 실제 각도 (실차 캘리브 대상)
...
- **타입이 상수 기본값에서 온다** — 정수 파라미터(`lane_poi.n_bands` 등)에 `4.5`를 주면 타입 불일치로 거부된다. `roi_frac`/`src_frac`/`car_center_px`류 tuple은 배열로: `ros2 param set /mission_node lane_poi.roi_frac "[0.6, 0.95]"`.
...
| 항목 | 뜻 | 캘리브 방법 |
|---|---|---|
| `src_frac` | BEV 원근변환 소스 사다리꼴(좌하,좌상,우상,우하 비율) | **최우선.** 기본값=무왜곡(사각형)이라 지금의 BEV는 진짜 호모그래피가 아니라 비등방 리사이즈다(아래 주의 참고). `--test <이미지>`로 BEV 출력을 보며 차선 2줄이 수직 평행이 되도록 좌상/우상의 x비율을 안쪽으로 좁힐 것(팀원 캘리브 절차와 동일) |
| `v_min_floor` | Otsu 적응 임계의 바닥값 | 트랙 조명에서 가장 어두운 프레임으로 시험 — 너무 낮으면 노이즈까지 흰색으로 잡힘, 너무 높으면 예전 고정임계(170)와 같은 실패 재현 |
| `px_per_m` | BEV 픽셀→미터 환산 | BEV에서 차선폭(실측 850mm 등)에 해당하는 픽셀 수로 역산. `src_frac` 캘리브 **후에** 할 것 |
| `ld_min_m`/`ld_max_m`/`ld_gain` | Pure-Pursuit lookahead 거리 | ⚠️ `ld`가 축거(0.545m)보다 짧으면 이득 `2L/ld`가 커져 조향각이 즉시 포화된다(사실상 bang-bang 회귀 + 속도까지 항상 바닥). 기본값은 `ld_min≈2·축거=1.1m`로 이득≈1을 맞춰둔 것 — 낮출 때는 `/debug/lane_poi`에 `SAT!`가 뜨지 않는지 확인 |
| `deg_per_pulse` | 조향 펄스(120ms) 1회당 실제 회전각 | lock→lock 걸리는 펄스 횟수를 세어 `STEERING_LIMIT_DEG*2 / 펄스횟수`로 역산 |
| `car_center_px` | BEV 하단 중앙(앞범퍼) 좌표 | `bev_w`/`bev_h`를 바꾸면 같이 갱신 (기본 `(bev_w/2, bev_h-1)`) |

**⚠️ `src_frac` 캘리브 전 알려진 한계 (2026-07-23 실측):** 기본 `src_frac`이 무왜곡 사각형이라, 현재 BEV는 세로 3.2배 확대(ROI 75행→240) vs 가로 0.5배 축소(640열→320)로 **축별 스케일이 6.4배 어긋나 있다.** 축 하나짜리 `px_per_m`으로는 원리적으로 둘 다 표현할 수 없으므로 전방거리(m)가 실제와 다르다. 진짜 사다리꼴로 캘리브하면 이 왜곡이 사라진다. 그 전까지도 **방향(좌/우)은 항상 맞고**(`alpha=atan2`가 픽셀 비율만 쓰므로 스케일이 약분됨) 조향각 "크기"만 영향을 받는다.

실차에서는 `/debug/lane_poi`의 lookahead 원(청록)과 `delta=` 텍스트를 보며 위 표 순서대로 튜닝할 것. 텍스트에 **`SAT!`**가 계속 뜨면 조향각이 한계에 붙어 비례 제어 이점이 사라진 상태이니 `ld_min_m`을 올리거나 `px_per_m`을 다시 잴 것.
...
| 카메라 열기 실패 (`VIDIOC_REQBUFS`, `can't open camera by index` 등) | 다른 프로그램이 점유 중인지 확인, WSL2면 [usbipd attach](#2단계-wsl2에서-실행-usb-연결). 그게 아니면 `front_camera` 기본값(자동 감지)이 다른 웹캠을 잘못 골랐을 수 있음(2026-07-17 실차: 내장/타사 웹캠이 `/dev/video0`을 먼저 차지) — **`python3 tools/pick_camera.py`로 각 카메라 프레임을 눈으로 확인하고 전방을 지정**하면 안정 식별자(by-id)로 저장돼 재부팅/재연결에도 유지된다(그 뒤로는 launch가 자동 사용). 일회성으로 바꾸려면 `front_camera:=<번호>` |
| 전/후방 카메라가 서로 바뀜 | 같은 모델(C920) 두 대면 자동 감지가 어느 쪽이 전방인지 알 수 없어 **찾은 순서대로** 배정한다 — `python3 tools/pick_camera.py`로 한 번 지정할 것. 되돌리려면 `config/cameras.json` 삭제 |
| 뭐가 문제인지 모르겠음 / 주행이 그냥 안 됨 | `python3 tools/doctor.py` — 정적 환경 → 디스크 → ROS 노드 → 토픽 생존 → 아두이노 → 카메라 → 라이다 → 미션 순서로 어디서 끊겼는지 짚어준다. 읽기 전용이라 모터는 움직이지 않는다(구동 확인은 `hw_test.py`) |
