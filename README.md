...
| `colcon build`에서 `autodrive_msgs`가 `ModuleNotFoundError: No module named 'em'`으로 실패 | Python 가상환경(`.venv` 등)이 활성화된 상태 — `deactivate` 후 재시도. ROS 2 전환 후에는 `.venv`가 필요 없다(이전 워크플로의 잔재라면 삭제해도 됨) |
