...
이 저장소는 ROS 2 패키지 하나(`autodrive_skku_ros`, 노드/미션 로직)를 담고 있다. 차량 제어 명령은 커스텀 메시지 없이 표준 `std_msgs`(Int16/String)만 쓴다 — 별도 메시지 패키지가 없으므로 rosidl 코드 생성 관련 빌드 문제도 없다. 콜콘 워크스페이스의 `src/` 아래 저장소 전체를 clone하면 빌드된다.
...
`/car/cmd/steer`(같은 방향 연속 발행은 무시, dedup)와 `/car/cmd/steer_pulse`(매번
강제로 한 펄스 더 보냄 — 차선 변경/T주차처럼 같은 방향 펄스를 여러 번 반복해야
하는 기동용)는 별도 토픽이다. 문자열 payload(`'F'`/`'L'`/`'R'`)는 실제로 아두이노에
그대로 전송되는 시리얼 바이트와 같다.
...
`hw_test.py`로도 안 움직이면 한 단계 더 내려가서 `car_controller.ino`의 시리얼
프로토콜/워치독 로직 자체를 배제하고 순수 GPIO 출력만 확인한다 —
`arduino/pin_test/pin_test.ino`는 `digitalWrite`/`analogWrite`만으로 LEFT →
RIGHT → STEER 순서로 각 채널을 1초씩 직접 돌린다(시리얼 명령 대기 없음, 전원
켜지자마자 바로 반복):
```bash
./tools/upload_firmware.sh /dev/ttyACM0 arduino/pin_test
```
이것도 안 움직이면 핀/배선/모터 드라이버 전원(아두이노 USB 전원과 별도로 모터
드라이버에 구동 전원이 들어가는지) 쪽 문제로 좁혀진다. 확인 후 반드시
`./tools/upload_firmware.sh`로 `car_controller.ino`를 다시 올릴 것.

하드웨어 공용 설정(포트, 카메라 인덱스, 기본 속도, 라이다 장착 보정 등)은 `autodrive_skku_ros/autodrive_skku_ros/config.py`에 있다. 미션별 튜닝값(차선 인식/장애물 감지/정지선/T주차 파라미터)은 그 값을 실제로 쓰는 `missions/*.py` 파일 상단에 있다 — 어떤 값이 어디 있는지는 아래 "저장소 구조" 참고. 둘 다 ROS 파라미터화하지 않고 파이썬 모듈 상수로 두었다(대회 전 회귀 위험 최소화).
...
`git pull --ff-only`가 실패하면(로컬에 커밋 안 된 실차 튜닝 등) 스크립트가 멈춘다 — 자동 머지·강제 덮어쓰기는 하지 않으므로 직접 `git status`로 확인 후 처리할 것. 파이썬 파일만 바뀐 경우 `colcon build --symlink-install`(setup.sh/update.sh가 이미 이 옵션으로 빌드함) 덕분에 재빌드 없이 바로 반영된다 — `setup.py`/`package.xml`(새 노드 실행파일 추가 등)이 바뀔 때만 다시 빌드가 필요하다.
...
| `/car/cmd/go`, `/car/cmd/stop` | `std_msgs/Empty` | 주행 허용 / 정지 (Foxglove Publish 패널 등에서 수동 명령 시) |
...
│   │   │   ├── arduino_node.py         #   시리얼 프로토콜 + /car/cmd/* 구독, /car/state 발행
...
각 `nodes/*.py`는 `python3 -m autodrive_skku_ros.nodes.<파일명> --selftest`로 ROS 없이
바로 검증할 수 있다 (예: `python3 -m autodrive_skku_ros.nodes.arduino_node --selftest`).
...
`arduino/car_controller/car_controller.ino`를 아두이노 메가 2560에 업로드한다 (외부 라이브러리 불필요). 실차 검증본 `run_test_fixed.ino`와 같은 핀맵이며, 가변 속도·후진·워치독이 추가됐다. **시리얼 프로토콜은 ROS 2 전환과 무관하게 그대로다** — `arduino_node.py`(실행: `ros2 run autodrive_skku_ros arduino_node`)의 `ArduinoBridgeNode`가 같은 파일의 `ArduinoNode` 클래스를 그대로 감싼다. `.ino`는 파이썬/ROS와 별개로 컴파일·업로드되는 펌웨어다 — 한 번 올리면 보드가 그 프로그램을 독립적으로 계속 실행하며, 파이썬 쪽은 이미 업로드된 보드와 시리얼로 통신만 한다(파이썬이 `.ino`를 실행/해석하지 않는다).

업로드 방법 두 가지:
- **WSL2 안에서 GUI 없이**: `./tools/upload_firmware.sh` (최초 실행 시 `arduino-cli` 설치 + `arduino:avr` 코어 설치까지 자동으로 함, 포트 생략하면 자동 감지)
...
1. `car_controller.ino` 업로드 (`./tools/upload_firmware.sh` 또는 Windows Arduino IDE — 위 '펌웨어' 절 참고. `run_test_fixed.ino` 대체, 같은 핀맵)
