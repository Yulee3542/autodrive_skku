...
`arduino/car_controller/car_controller.ino`를 아두이노 메가 2560에 업로드한다 (외부 라이브러리 불필요). 실차 검증본 `run_test_fixed.ino`와 같은 핀맵이며, 가변 속도·후진·워치독이 추가됐다. **시리얼 프로토콜은 ROS 2 전환과 무관하게 그대로다** — `arduino_bridge_node`가 기존 `ArduinoNode` 클래스를 그대로 감싼다. `.ino`는 파이썬/ROS와 별개로 컴파일·업로드되는 펌웨어다 — 한 번 올리면 보드가 그 프로그램을 독립적으로 계속 실행하며, 파이썬 쪽은 이미 업로드된 보드와 시리얼로 통신만 한다(파이썬이 `.ino`를 실행/해석하지 않는다).
