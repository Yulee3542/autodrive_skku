...
| 항목 | 뜻 | 캘리브 방법 |
|---|---|---|
| `src_frac` | BEV 원근변환 소스 사다리꼴(좌하,좌상,우상,우하 비율) | 기본값=무왜곡(사각형). `--test <이미지>`로 BEV 출력을 보며 차선 2줄이 수직 평행이 되도록 좌상/우상의 x비율을 안쪽으로 좁힐 것(팀원 캘리브 절차와 동일) |
| `v_min_floor` | Otsu 적응 임계의 바닥값 | 트랙 조명에서 가장 어두운 프레임으로 시험 — 너무 낮으면 노이즈까지 흰색으로 잡힘, 너무 높으면 예전 고정임계(170)와 같은 실패 재현 |
| `px_per_m` | BEV 픽셀→미터 환산 | BEV에서 차선폭(실측 850mm 등)에 해당하는 픽셀 수로 역산 |
...
캘리브 전에도 방향(좌/우)은 항상 맞고(`STEERING_LIMIT_DEG`로 클램프됨), 크기만 부정확할 수 있다 — 실차에서 `/debug/lane_poi`의 lookahead 원(청록)과 `delta=` 텍스트를 보며 위 표 순서대로 튜닝할 것.
